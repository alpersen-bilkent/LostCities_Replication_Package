# -*- coding: utf-8 -*-
"""
Created on Sat Jan 31 15:30:13 2026
...
def draw_confidence_ellipse(pts, ax, n_std=1.0, edgecolor='red', label=None, **kwargs):
    """
    Plots a confidence ellipse based on the covariance of pts (N, 2).
    """
    ...
    # Width and height are diameters, hence 2 * n_std * sqrt(eigenvalue)
...
for b in tqdm(range(N_BOOTSTRAP)):
    ...
    b_traffic['Traffic'] = b_traffic['Traffic'].replace(0, 0.5)

    all_trade = pd.concat([b_traffic[['City1', 'Traffic']].rename(columns={'City1': 'City'}), 
                           b_traffic[['City2', 'Traffic']].rename(columns={'City2': 'City'})])
    b_totals = all_trade.groupby('City')['Traffic'].sum().to_dict()
    
    b_sim = pd.DataFrame(0.0, index=all_cities, columns=all_cities)
    for _, row in b_traffic.iterrows():
        c1, c2, vol = row['City1'], row['City2'], row['Traffic']
        if c1 in all_cities and c2 in all_cities:
            share = 0.5 * ((vol / b_totals.get(c1, 1e-9)) + (vol / b_totals.get(c2, 1e-9)))
            b_sim.loc[c1, c2] += share
            b_sim.loc[c2, c1] += share

    with np.errstate(divide='ignore'):
        b_dissim = np.sqrt(1.0 / b_sim).replace([np.inf, -np.inf], 100.0)
...
# --- Step 4: Individual City Confidence Plots ---
print("\nGenerating Individual Confidence Plots...")
for name in city_print_order:
    ...
    # Comparison Points
    ax.scatter(np.mean(pts[:,0]), np.mean(pts[:,1]), c='red', marker='X', s=120, label='MDS Mean', edgecolors='k', zorder=6)
    ...
    # Zooming logic
    pad = 0.8
    all_x = [pts[:,0].min(), pts[:,0].max(), baj['est_long'], act['long_x']]
    all_y = [pts[:,1].min(), pts[:,1].max(), baj['est_lat'], act['lat_y']]
    ax.set_xlim(min(all_x) - pad, max(all_x) + pad)
    ax.set_ylim(min(all_y) - pad, max(all_y) + pad)
    
    ax.set_title(f"Position Confidence: {name} (Error: {results_df.loc[results_df['city']==name, 'error_km'].values[0]:.1f}km)")
    ...
    # --- Step 5: Summary Comparison Plot (The "Everything" Style) ---
...
ax.scatter(bajramovic_df['est_long'], bajramovic_df['est_lat'], 
           c='green', marker='s', s=80, label='Bajramovic Est.', edgecolors='k', zorder=4)

# Plot Error Lines (MDS)
for index, row in results_df.iterrows():
    ax.plot([row['actual_long'], row['mean_long']], [row['actual_lat'], row['mean_lat']], 'r--', alpha=0.5)
    
    # Custom Annotation logic
    if row['city'] == 'Hattus':
        ax.text(row['actual_long'], row['actual_lat'] - 0.2, row['city'], fontsize=8, color='blue', ha='center')
    else:
        ax.text(row['actual_long'], row['actual_lat'] + 0.05, row['city'], fontsize=8, color='blue')

# Plot Error Lines (Bajramovic)
