# -*- coding: utf-8 -*-
"""
Created on Fri Jan 30 23:06:23 2026
Modern Turkey Cities Geolocalization (GCN) with Bootstrapping
Method: Linear Dyadic Dependence with Poisson Resampling
@author: user
"""
...
Modern Turkey Cities Geolocalization (GCN) with Bootstrapping
Method: Linear Dyadic Dependence with Poisson Resampling
Fix: Added scaling logic for high-volume trade data to prevent Poisson overflow
"""
...
# File paths - Ensure these are correct for your local machine
...
symmetric_matrix = (df_subset + df_subset.T).values.astype(np.float32)
...
print(f"Executing Joint Monte Carlo Sampling ({N_BOOTSTRAP} seeds x {num_cities} LOO)...")

# FIX: Scaling factor to prevent Poisson 'lam value too large' error
# 1,000,000 is high enough for precision but low enough for the sampler
max_val = symmetric_matrix.max()
scaling_factor = 1e3 / max_val if max_val > 1e3 else 1.0
scaled_base_matrix = symmetric_matrix * scaling_factor

for seed_idx, seed in enumerate(tqdm(BOOTSTRAP_SEEDS, desc="Monte Carlo Iterations")):
    # Sample from scaled matrix
    resampled_traffic = np.random.poisson(scaled_base_matrix).astype(float)
    ...
    for i in range(num_cities):
        ...
        graph_data = create_graph_data(resampled_traffic, num_cities)
...
# --- 4. Statistics and Table Generation ---
table_rows = []
...
for name in valid_cities:
    ...
    # Dynamic zoom for modern cities
    pad = 0.5
...
# --- 6. Final Aggregate Map ---
fig, ax = plt.subplots(figsize=(14, 10))
load_and_plot_turkey_map(ax)
# Zoom level adjusted for Central/Eastern Anatolia subset
...
ax.set_title(f'Modern Turkey: Bootstrapped GCN Means\nOverall Mean Error: {results_final_df["Error_KM"].mean():.2f} km', fontsize=16)
plt.show()

print("\n--- BOOTSTRAPPED VALIDATION RESULTS (MODERN CITIES) ---")
