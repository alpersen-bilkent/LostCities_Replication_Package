# -*- coding: utf-8 -*-
"""
Created on Sat Jan 31 17:24:00 2026
@author: user
Modern Turkish Cities - TII Methodology Congruent with Unified Style
"""
...
# --- 4. BOOTSTRAP LOOP (TII Normalization) ---
...
print(f"\nStarting {N_BOOTSTRAP} Bootstrap Runs with TII & Poisson Scaling...")
for b in tqdm(range(N_BOOTSTRAP)):
    ...
    # 2. Symmetric TII Similarity
    x_iW = b_vol.sum(axis=1) # Exports
    x_Wj = b_vol.sum(axis=0) # Imports
    x_WW = b_vol.sum()       # Total World
    
    # TII_ij = (x_ij * x_WW) / (x_iW * x_Wj)
    T_ij = (b_vol * x_WW) / (np.outer(x_iW, x_Wj) + 1e-9)
    T_ji = (b_vol.T * x_WW) / (np.outer(x_Wj, x_iW) + 1e-9)
    b_sim_arr = 0.5 * (T_ij + T_ji)
    ...
    # 5. Weighted Procrustes Alignment
...
print("\n" + "="*125)
print(f"FINAL BOOTSTRAP RESULTS ({N_BOOTSTRAP} runs - TII Methodology)")
print("="*125)
...
for name in all_cities_list:
    ...
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1), fontsize='medium', title="Legend", title_fontsize='large', framealpha=1)
...
ax.scatter(results_df['mean_long'], results_df['mean_lat'], c='red', marker='X', s=100, label='MDS Est.', edgecolors='k', zorder=5)

for _, row in results_df.iterrows():
    ax.plot([row['actual_long'], row['mean_long']], [row['actual_lat'], row['mean_lat']], 'r--', alpha=0.5, zorder=4)
    ax.text(row['actual_long'], row['actual_lat'] + 0.05, row['city'], fontsize=8, color='blue', fontweight='bold')

ax.set_title(f'Actual vs. MDS Bootstrap Estimates (Modern Cities - TII)\nOverall Mean Error: {results_df["error_km"].mean():.2f} km', fontsize=16)
