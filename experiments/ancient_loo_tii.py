# -*- coding: utf-8 -*-
"""
15 fictitiously-held-out known ancient cities, LOO, TII weights.
Post-Bug-2-fix: trains on the full 25-node graph.

Corresponds to legacy/GCNTIIBootstrap.py. LR=0.011, EMBEDDING_DIM=8
unchanged by the bug fixes (this pair was actually competed against
alternatives in the re-search, not just re-confirmed by default -- see
grid_search_ancient_tii.py). EPOCHS updated 244 -> 62 per that same
search, run on the corrected 25-node graph. N_BOOTSTRAP=200 -- raised
from 40 to give the significance test
(evaluate.bootstrap_ensemble_mean_distribution()) enough replicates for
reliable percentile estimation, and to match N_BOOTSTRAP already used in
the lost-city prediction scripts.
"""
...
EXPERIMENT_NAME = "ancient_loo_tii"
MODEL_LABEL = "TII"
CATEGORY = "LOO"
HPARAMS = {"epochs": 62, "lr": 0.011, "embedding_dim": 8}
N_BOOTSTRAP = 200


def main():
    summary_df, raw_results = evaluate.run_ancient_loo("tii", HPARAMS, N_BOOTSTRAP)
